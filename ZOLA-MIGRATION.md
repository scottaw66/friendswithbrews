...
- [ ] `base.html` (head parity: title, meta, view-transition meta, favicon
      links, font preload, RSS alternate link, Fathom script).
- [ ] Partials: header, menu, footer (correct year — see bug note).
- [ ] Components: episode_article, episode_card, brew_details,
      podcast_links, video_embed (lite-youtube), transcript views, reviews.
- [ ] `icons.html` with the ~20 inline SVG icon components (`size`, `fill`
      props matching astro-icon's rendered output).
- [ ] Pages: `/`, `/N/` episode pages, `/friends/`, `/follow/`.
- [ ] Parity: byte-diff `/101/` and `/` against baseline (modulo hashed
      asset paths and scoped-style attributes).
